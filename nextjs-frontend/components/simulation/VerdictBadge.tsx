...
const CONFIG = {
  추천: { icon: "✅", label: "추천", className: "bg-dalock-success text-white", rank: (p: number) => `상위 ${Math.max(1, Math.round(100 - p))}%` },
  검토필요: { icon: "⚠️", label: "검토필요", className: "bg-dalock-warning text-white", rank: (p: number) => `상위 ${Math.max(1, Math.round(100 - p))}%` },
  비추천: { icon: "❌", label: "비추천", className: "bg-dalock-danger text-white", rank: (p: number) => `하위 ${Math.max(1, Math.round(p))}%` },
} as const;
